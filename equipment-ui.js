...
export function loadoutDialog(entry,data,ui){
 ...
 function statsPanel(){
  ...
  function facing(a,parent){const row=el('div',undefined,'armor-facing');row.append(el('span',t(a.side===4?'Frontal':a.side===5?'Rear':sides[a.side]),'facing-name'));const values=el('div',undefined,'armor-values');
   ...
   row.append(values);parent.append(row);
  }
